...
procedure advent_02_2 is
  ...
  Lose_Table: constant array (Shape) of Shape :=
    (Rock => Scissors, Paper => Rock, Scissors => Paper);
  Win_Table: constant array (Shape) of Shape :=
  ...
  begin
    ...
    case S(S'First + 2) is
      when 'X' => myself := Lose_Table(opponent);
      when 'Y' => myself := opponent;
      when 'Z' => myself := Win_Table(opponent);
      when others => raise Constraint_Error;
    end case;
